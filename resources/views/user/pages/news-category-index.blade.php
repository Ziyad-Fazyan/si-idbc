...
@section('content')
                ...
                <form action="{{ route($prefix . 'news.category-store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="bg-white rounded-lg shadow-md overflow-hidden">
                        <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200">
                            ...
                            <button type="submit" class="inline-flex items-center px-3 py-2 border border-[#0C6E71] text-[#0C6E71] hover:bg-[#0C6E71] hover:text-white rounded transition-colors duration-200">
                        ...
                        <div class="p-6">
                            ...
                            <div class="mb-2">
                                <label for="desc" class="block text-sm font-medium text-gray-700 mb-1">Deskripsi Kategori Berita</label>
                                <textarea name="desc" 
                                          id="desc" 
                                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#0C6E71] focus:border-[#0C6E71]" 
                                          placeholder="Inputkan Deskripsi Kategori Berita"
                                          rows="4"></textarea>
                                @error('desc')
                                    <small class="text-red-500 text-xs mt-1">{{ $message }}</small>
                                @enderror
                        ...
                        <table class="min-w-full divide-y divide-gray-200" id="categoryTable">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Kategori</th>
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Kode</th>
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Slug</th>
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Deskripsi</th>
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Button</th>
                                </tr>
                            </thead>
    ...
    @foreach ($category as $item)
        <div id="editModal{{ $item->code }}" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
                ...
                <form action="{{ route($prefix . 'news.category-update', $item->code) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200">
                        ...
                        <div class="flex space-x-2">
                            <button type="submit" class="px-3 py-1 border border-[#0C6E71] text-[#0C6E71] hover:bg-[#0C6E71] hover:text-white rounded transition-colors duration-200">
                                <i class="fas fa-paper-plane"></i>
                            </button>
                            <button type="button" onclick="closeEditModal('{{ $item->code }}')" class="px-3 py-1 border border-red-500 text-red-500 hover:bg-red-500 hover:text-white rounded transition-colors duration-200">
                    ...
                    <div class="p-6">
                        <div class="mb-4">
                            <label for="edit_name{{ $item->code }}" class="block text-sm font-medium text-gray-700 mb-1">Nama Kategori</label>
                            <input type="text" 
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#0C6E71] focus:border-[#0C6E71]" 
                                   name="name" 
                                   id="edit_name{{ $item->code }}"
                                   value="{{ $item->name }}"
                                   placeholder="Inputkan nama kategori berita...">
                            @error('name')
                                <small class="text-red-500 text-xs mt-1">{{ $message }}</small>
                        ...
                        <div class="mb-2">
                            <label for="edit_desc{{ $item->code }}" class="block text-sm font-medium text-gray-700 mb-1">Deskripsi Kategori Berita</label>
                            <textarea name="desc" 
                                      id="edit_desc{{ $item->code }}" 
                                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#0C6E71] focus:border-[#0C6E71]" 
                                      placeholder="Inputkan Deskripsi Kategori Berita"
                                      rows="4">{{ $item->desc }}</textarea>
                            @error('desc')
                                <small class="text-red-500 text-xs mt-1">{{ $message }}</small>
                            @enderror
                        </div>
    ...
    <div id="deleteModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-lg p-6 max-w-sm w-full mx-4">
            <div class="text-center">
                ...
                <p class="text-gray-500 mb-6">Apakah Anda yakin ingin menghapus kategori <span id="deleteItemName" class="font-semibold"></span>?</p>
