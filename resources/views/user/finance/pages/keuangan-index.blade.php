...
@section('content')
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="value" id="value"
                                placeholder="Inputkan nominal balance...">
                            @error('value')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <textarea name="desc" id="desc" class="form-control" cols="30" rows="10"
                                placeholder="Inputkan deskripsi sumber dana..."></textarea>
                            @error('desc')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                                ...
                                <tr class="">
                                    ...
                                    <td class="d-flex justify-content-center align-items-center">
                                        <a href="#" style="margin-right: 10px" data-bs-toggle="modal"
                                            data-bs-target="#updateFakultas{{ $item->code }}"
                                        ...
                                        <form id="delete-form-{{ $item->code }}"
                                            action="{{ route($prefix . 'finance.keuangan-destroy', $item->code) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <a type="button" class="bs-tooltip btn btn-rounded btn-outline-danger"
                                                ...
                                                data-url="{{ route($prefix . 'finance.keuangan-destroy', $item->code) }}"
                                                data-name="{{ $item->name }}"
    ...
    <div class="me-1 mb-1 d-inline-block">
        ...
        @foreach ($balance as $item)
            <form action="{{ route($prefix . 'finance.keuangan-update', $item->code) }}" method="POST"
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <select name="type" id="type" class="form-select">
                                        <option value="" selected>Pilih Type Keuangan</option>
                                        <option value="0" {{ $item->raw_type == 0 ? 'selected' : '' }}>Balance
                                            Pending</option>
                                        <option value="1" {{ $item->raw_type == 1 ? 'selected' : '' }}>Balance Income
                                        </option>
                                        <option value="2" {{ $item->raw_type == 2 ? 'selected' : '' }}>Balance
                                            Expenses</option>
                                    </select>
                                    @error('type')
                                        <small class="text-danger">{{ $message }}</small>
                                ...
                                <div class="form-group">
                                    <label for="value">Nominal Balance</label>
                                    <input type="text" class="form-control" name="value" id="value"
                                        value="{{ $item->value }}" placeholder="Inputkan nominal balance...">
                                    @error('value')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    ...
                                    <textarea name="desc" id="desc" cols="30" rows="10" class="form-control"
                                        value="{{ $item->desc }}" placeholder="Inputkan deskripsi sumber dana...">{{ $item->desc }}</textarea>
                                    @error('desc')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
