        ...
        <div class="card">
            <div class="card-header">
                <h5 class="card-title d-flex justify-content-between align-items-center">
                    @yield('submenu')
                    <div class="">
                        <a href="{{ route($prefix . 'master.kelas-index') }}" class="btn btn-outline-warning mt-1"><i
                                class="fa-solid fa-backward"></i></a>
                        <form action="{{ route($prefix . 'master.kelas-mahasiswa-cetak', $kelas->code) }}" method="post">
                            @csrf
                            <button type="submit" style="margin-right: 10px" class="btn btn-outline-danger mt-1"><i
                        ...
                        @foreach ($mahasiswa as $key => $item)
                            <tr>
                                ...
                                @php
                                    ...
                                    // dd($absen->count());
                                @endphp
                                <td data-label="Data Kehadiran">{{ $absen->count() }} / {{ $jadkul->count() }} Perkuliahan
                                </td>
                                <td data-label="Presentase Kehadiran">{{ ($absen->count() / $jadkul->count()) * 100 }} %
                                </td>
                    ...
                    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h4 class="modal-title" id="myModalLabel16">Edit Data Mahasiswa - {{ $item->mhs_name }}
                                </h4>
                                <div class="">
