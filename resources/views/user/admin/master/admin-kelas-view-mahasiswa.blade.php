            ...
            <div class="p-4 border-b border-gray-200">
                <h5 class="flex justify-between items-center text-lg font-semibold">
                    ...
                    <div class="flex gap-2">
                        ...
                        <form action="{{ route($prefix . 'master.kelas-mahasiswa-cetak', $kelas->code) }}" method="post" class="inline">
                            @csrf
                            <button type="submit" 
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Nomor NIM</th>
                            <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Mahasiswa</th>
                            ...
                            <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Presentase Kehadiran</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($mahasiswa as $key => $item)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-900 text-center">{{ $item->mhs_gend == null ? '-' : $item->mhs_gend }}</td>
                                @php
                                ...
                                @endphp
                                <td class="px-4 py-3 text-sm text-gray-900 text-center">{{ $absen->count() }} / {{ $jadkul->count() }} Perkuliahan</td>
                                <td class="px-4 py-3 text-sm text-gray-900 text-center">{{ ($absen->count() / $jadkul->count()) * 100 }} %</td>
                                <td class="px-4 py-3 text-sm text-center">
                                    <button type="button" 
                                            onclick="openModal('editMahasiswa{{ $item->code }}')"
                                            class="inline-flex items-center px-3 py-2 border border-[#0C6E71] text-[#0C6E71] rounded-md hover:bg-[#0C6E71] hover:text-white transition-colors duration-300">
                    ...
                    <form action="#" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="flex justify-between items-center pb-4 mb-4 border-b border-gray-200">
                                <h3 class="text-lg font-semibold text-gray-900">Edit Data Mahasiswa - {{ $item->mhs_name }}</h3>
                                <div class="flex gap-2">
                                    <button type="submit" class="inline-flex items-center px-3 py-2 border border-[#0C6E71] text-[#0C6E71] rounded-md hover:bg-[#0C6E71] hover:text-white transition-colors duration-300">
                                        <i class="fas fa-paper-plane"></i>
                                    </button>
                                    <button type="button" onclick="closeModal('editMahasiswa{{ $item->code }}')" class="inline-flex items-center px-3 py-2 border border-[#FF6B35] text-[#FF6B35] rounded-md hover:bg-[#FF6B35] hover:text-white transition-colors duration-300">
