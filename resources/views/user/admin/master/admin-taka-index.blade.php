...
@section('content')
                ...
                <form action="{{ route($prefix . 'master.taka-store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
                        ...
                        <div class="p-4 space-y-4">
                            <div class="space-y-2">
                                <label for="name" class="block text-sm font-medium text-gray-700">Nama Tahun Akademik</label>
                            ...
                            <div class="space-y-2">
                                <label for="code" class="block text-sm font-medium text-gray-700">Kode Fakultas ( 6 Angka )</label>
                                <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0C6E71] focus:border-[#0C6E71]" 
                                    name="code" id="code" placeholder="Inputkan kode tahun akademik..." maxlength="6" uppercase
                            ...
                            <div class="space-y-2">
                                <label for="semester" class="block text-sm font-medium text-gray-700">Semester Perkuliahan</label>
                                <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0C6E71] focus:border-[#0C6E71]" 
                            ...
                            <div class="space-y-2">
                                <label for="year_start" class="block text-sm font-medium text-gray-700">Pilih Tahun Masuk</label>
                            ...
                            <table class="min-w-full divide-y divide-gray-200" id="table1">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Tahun Akademik</th>
                                        <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Kode Tahun Akademik</th>
                                        <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Semester Perkuliahan</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($taka as $key => $item)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-4 py-3 text-center text-sm text-gray-500">{{ $item->semester }}</td>
                                    ...
                                    <td class="px-4 py-3 text-center text-sm text-gray-500">
                                        <div class="flex justify-center items-center space-x-2">
                                            ...
                                            <form id="delete-form-{{ $item->code }}"
                                                action="{{ route($prefix . 'master.taka-destroy', $item->code) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" 
    ...
    @foreach ($taka as $item)
        <div id="updateTaka{{ $item->code }}" class="fixed inset-0 z-50 hidden overflow-auto bg-black bg-opacity-50 flex items-center justify-center p-4">
                ...
                <form action="{{ route($prefix . 'master.taka-update', $item->code) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-4 space-y-4">
                        <div class="space-y-2">
                            <label for="name" class="block text-sm font-medium text-gray-700">Nama Tahun Akademik</label>
                            <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0C6E71] focus:border-[#0C6E71]" 
                                name="name" id="name" placeholder="Inputkan nama tahun akademik..." value="{{ $item->name }}">
                            @error('name')
                                <small class="text-red-500">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="space-y-2">
                            ...
                            <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0C6E71] focus:border-[#0C6E71]" 
                                name="code" id="code" placeholder="Inputkan kode tahun akademik..." maxlength="6" uppercase
                                onkeydown="return /[a-zA-Z0-9]/i.test(event.key)" value="{{ $item->code }}">
                            @error('code')
                                <small class="text-red-500">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="space-y-2">
                            <label for="semester" class="block text-sm font-medium text-gray-700">Semester Perkuliahan</label>
                            <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0C6E71] focus:border-[#0C6E71]" 
                                name="semester" id="semester" placeholder="Inputkan kode tahun akademik..." value="{{ $item->semester }}">
                            @error('semester')
                                <small class="text-red-500">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-2">
                            <label for="is_active" class="block text-sm font-medium text-gray-700">Status Tahun Akademik</label>
                            <select name="is_active" id="is_active" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0C6E71] focus:border-[#0C6E71]">
