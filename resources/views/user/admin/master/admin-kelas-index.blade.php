...
@section('content')
        ...
        <div class="col-lg-12 col-12">
            <div class="card">
                <div class="card-header d-flex align-items-center justify-content-between">
                    ...
                    <a href="#" style="margin-right: 10px" data-bs-toggle="modal" data-bs-target="#tambahKelas"
                        ...
                        <tbody>
                            @foreach ($kelas as $key => $item)
                                <tr class="text-center">
                                    ...
                                    <td data-label="Program Studi">{{ $item->pstudi->name . ' - ' . $item->taka->semester }}
                                    </td>
                                    ...
                                    <td class="d-flex justify-content-center align-items-center">
                                        ...
                                        <a href="{{ route($prefix . 'master.kelas-mahasiswa-view', $item->code) }}"
                                            style="margin-right: 10px" class="btn btn-outline-info"><i
                            ...
                            <div class="row">
                                <div class="form-group col-lg-4 col-12">
                                    ...
                                    <input type="text" class="form-control" name="name" id="name"
                                        placeholder="Inputkan nama Kelas...">
                                    @error('name')
                                ...
                                <div class="form-group col-lg-4 col-12">
                                    ...
                                    <input type="text" class="form-control" name="code" id="code"
                                        placeholder="Inputkan kode Kelas..." maxlength="32">
                                    @error('code')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
                                <div class="form-group col-lg-4 col-12">
                                    ...
                                    <input type="number" class="form-control" name="capacity" id="capacity"
                                        placeholder="Inputkan kode Kelas..." max="35" maxlength="2">
                                    @error('capacity')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
                                </div>
    ...
    <div class="me-1 mb-1 d-inline-block">
        ...
        @foreach ($kelas as $item)
            <form action="{{ route($prefix . 'master.kelas-update', $item->code) }}" method="POST"
                enctype="multipart/form-data">
                @method('patch')
                @csrf
                <div class="modal fade text-left w-100" id="updateKelas{{ $item->code }}" tabindex="-1"
                            ...
                            <div class="modal-body">
                                <div class="row">
                                    <div class="form-group col-lg-4 col-12">
                                        <label for="name">Nama Kelas</label>
                                        <input type="text" class="form-control" name="name" id="name"
                                            placeholder="Inputkan nama Kelas..." value="{{ $item->name }}">
                                        @error('name')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                                    <div class="form-group col-lg-4 col-12">
                                        <label for="code">Kode Kelas</label>
                                        <input type="text" class="form-control" name="code" id="code"
                                            placeholder="Inputkan kode Kelas..." maxlength="32" uppercase
                                            value="{{ $item->code }}">
                                        @error('code')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    ...
                                    <div class="form-group col-lg-4 col-12">
                                        <label for="capacity">Kapasitas Kelas</label>
                                        <input type="number" class="form-control" name="capacity" id="capacity"
                                            placeholder="Inputkan kode Kelas..." max="35" maxlength="2" uppercase
                                            value="{{ $item->capacity }}">
                                        @error('capacity')
                                            <small class="text-danger">{{ $message }}</small>
                                        @enderror
                                    ...
                                    <div class="form-group col-lg-6 col-12">
                                        ...
                                        <select name="taka_id" id="taka_id" class="form-select">
                                            ...
                                            @foreach ($taka as $taaka)
                                                <option value="{{ $taaka->id }}"
                                                    {{ $item->taka_id == $taaka->id ? 'selected' : '' }}>
                                                    {{ $taaka->name . ' - ' . $taaka->semester }}</option>
                                            @endforeach
                                        </select>
                                        @error('taka_id')
                                            <small class="text-danger">{{ $message }}</small>
                                    ...
                                    <div class="form-group col-lg-6 col-12">
                                        ...
                                        <select name="proku_id" id="proku_id" class="form-select">
                                            ...
                                            @foreach ($proku as $prokuh)
                                                <option value="{{ $prokuh->id }}"
                                                    {{ $item->proku_id == $prokuh->id ? 'selected' : '' }}>
                                                    {{ $prokuh->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('proku_id')
                                        ...
                                        <select name="dosen_id" id="dosen_id" class="form-select">
                                            ...
                                            @foreach ($dosen as $dosens)
                                                <option value="{{ $dosens->id }}"
                                                    {{ $item->dosen_id == $dosens->id ? 'selected' : '' }}>
                                                    {{ $dosens->dsn_name }}</option>
                                            @endforeach
                                        </select>
                                        @error('dosen_id')
                                            <small class="text-danger">{{ $message }}</small>
