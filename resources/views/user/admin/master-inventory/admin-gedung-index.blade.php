            ...
            <form action="{{ route($prefix . 'inventory.gedung-store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="bg-white rounded-lg shadow-md overflow-hidden">
                    <div class="px-6 py-4 flex justify-between items-center border-b border-gray-200">
                        ...
                        <button type="submit"
                            class="px-3 py-2 border border-[#0C6E71] text-[#0C6E71] rounded-md hover:bg-[#0C6E71] hover:text-white transition-colors duration-300">
                    ...
                    <div class="p-6">
                        ...
                        <div class="mb-4">
                            <label for="code" class="block text-sm font-medium text-gray-700 mb-1">Kode Gedung ( 3 Huruf
                                Kapital )</label>
                            <input type="text"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0C6E71] focus:border-[#0C6E71]"
                                name="code" id="code" placeholder="Inputkan kode gedung..." maxlength="3" uppercase
                                ...
                                @foreach ($gedung as $key => $item)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500"
                                            data-label="Number">{{ ++$key }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500"
                                            data-label="Nama Gedung">{{ $item->name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500"
                                            data-label="Kode Gedung">{{ $item->code }}</td>
                        ...
                        @foreach ($gedung as $key => $item)
                            <div class="bg-white shadow rounded-lg mb-4 p-4 border-l-4 border-[#0C6E71]">
                                <div class="flex justify-between items-center mb-2">
                                    ...
                                    <span
                                        class="bg-gray-100 text-gray-800 text-xs font-medium px-2.5 py-0.5 rounded">{{ $item->code }}</span>
                                </div>
                ...
                <div
                    ...
                    <form action="{{ route($prefix . 'inventory.gedung-update', $item->code) }}" method="POST"
                            ...
                            <div class="sm:flex sm:items-start">
                                <div class="w-full">
                                    <div class="mt-3 text-center sm:mt-0 sm:text-left">
                                        <h3 class="text-lg leading-6 font-medium text-gray-900">Edit Gedung -
                                            {{ $item->name }}</h3>
                                        <div class="mt-4">
                                            <div class="mb-4">
                                            ...
                                            <div class="mb-4">
                                                <label for="edit-code-{{ $item->code }}"
                                                    class="block text-sm font-medium text-gray-700 mb-1">Kode Gedung ( 3
                                                    Huruf Kapital )</label>
                                                <input type="text"
                                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0C6E71] focus:border-[#0C6E71]"
                                                    name="code" id="edit-code-{{ $item->code }}"
                                                    placeholder="Inputkan kode gedung..." value="{{ $item->code }}"
                                                    maxlength="3" uppercase
                                                    onkeydown="return /[a-zA-Z]/i.test(event.key)">
                                                @error('code')
                                                    <small class="text-red-500 text-xs">{{ $message }}</small>
                                                @enderror
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            <button type="submit"
                                class="w-full inline-flex justify-center rounded-md border border-transparent px-4 py-2 bg-[#0C6E71] text-white text-base font-medium hover:bg-[#095052] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#0C6E71] sm:ml-3 sm:w-auto sm:text-sm">
                                <i class="fas fa-paper-plane mr-2"></i> Submit
                            </button>
                            <button type="button"
                                class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
