            ...
            <div class="p-6 border-b border-gray-100 bg-gray-50/50">
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                    <div>
                        ...
                        <p class="text-gray-600 text-sm mt-1">Kelola data pengguna mahasiswa sistem</p>
                    </div>
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">NIM
                            </th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                Nama Mahasiswa</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                        ...
                        @forelse ($student as $key => $item)
                            <tr class="hover:bg-gray-50/50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $item->mhs_nim }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $item->mhs_name }}</div>
                                    <div class="text-sm text-gray-500">{{ $item->mhs_code }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @forelse($item->kelas as $kelas)
                                        <span
                                            class="inline-flex px-2.5 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800 mr-1 mb-1">
                                            {{ $kelas->name }}
                                        </span>
                                    @empty
                                        <span class="text-xs text-gray-500">Tidak ada kelas</span>
                                    @endforelse
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $item->mhs_gend }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        ...
                                        {{ $item->mhs_stat === 'Aktif' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        {{ $item->mhs_stat }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center space-x-2">
                                        <button
                                            onclick="openContactModal('{{ $item->mhs_code }}', '{{ $item->mhs_name }}', '{{ $item->mhs_phone }}', '{{ $item->mhs_mail }}')"
                                        ...
                                        <form action="{{ route('web-admin.workers.student-destroy', $item->mhs_code) }}"
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center text-gray-500">
                                        ...
                                        <p class="text-sm">Mulai dengan menambahkan mahasiswa pertama</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="bg-white rounded-xl shadow-xl w-full max-w-md transform transition-all">
                <form action="{{ route($prefix . 'services.convert.import-student') }}" method="POST"
                    ...
                    <div class="flex items-center justify-between p-6 border-b border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-900">Import Data Mahasiswa</h3>
                            ...
                            <div class="flex items-start">
                                ...
                                <div class="text-sm text-blue-700">
                                    ...
                                    <ul class="text-xs space-y-1">
                                        ...
                                        <li>• File CSV (.csv)</li>
                                        <li>• <a href="#" class="underline hover:no-underline">Download template</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
    ...
    <div id="contactModal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-md transform transition-all">
                <!-- Modal Header -->
                <div class="flex items-center justify-between p-6 border-b border-gray-200">
                    <h3 id="contactModalTitle" class="text-lg font-semibold text-gray-900">Kontak Mahasiswa</h3>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('click', function(event) {
                if (event.target.classList.contains('fixed') && event.target.classList.contains(
                    'inset-0')) {
