...
@section('content')
            ...
            <div class="p-6 border-b border-gray-100 bg-gray-50/50">
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                    <div>
                        ...
                        <p class="text-gray-600 text-sm mt-1">Kelola data pengguna dosen sistem</p>
                    </div>
                    <div class="flex flex-wrap gap-3">
                        ...
                        <button type="button" x-data @click="$dispatch('open-modal', {name: 'export-dosens'})"
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">NIDN</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Nama Dosen</th>
                        ...
                        @forelse ($dosen as $key => $item)
                            <tr class="hover:bg-gray-50/50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $item->dsn_nidn }}</div>
                                    <div class="text-sm text-gray-500">{{ $item->dsn_code }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $item->dsn_name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex px-2.5 py-1 rounded-full text-xs font-medium
                                        {{ $item->dsn_gend === 'Laki-laki' ? 'bg-blue-100 text-blue-800' : 'bg-pink-100 text-pink-800' }}">
                                        {{ $item->dsn_gend }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex px-2.5 py-1 rounded-full text-xs font-medium
                                        {{ $item->raw_dsn_stat === 1 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        {{ $item->raw_dsn_stat === 1 ? 'Aktif' : 'Non-Aktif' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center space-x-2">
                                        <button
                                            onclick="openContactModal('{{ $item->dsn_code }}', '{{ $item->dsn_name }}', '{{ $item->dsn_phone }}', '{{ $item->dsn_mail }}')"
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center text-gray-500">
                                        ...
                                        <p class="text-sm">Mulai dengan menambahkan dosen pertama</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="bg-white rounded-xl shadow-xl w-full max-w-md transform transition-all">
                <form action="{{ route('web-admin.services.convert.import-users') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex items-center justify-between p-6 border-b border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-900">Import Data Dosen</h3>
    ...
    <div id="contactModal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-md transform transition-all">
                <!-- Modal Header -->
                <div class="flex items-center justify-between p-6 border-b border-gray-200">
                    <h3 id="contactModalTitle" class="text-lg font-semibold text-gray-900">Kontak Dosen</h3>
    ...
    <x-modal name="export-dosens">
        @include('user.admin.pages.modal.export-dosens')
    </x-modal>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('form').forEach(form => {
                form.addEventListener('submit', function() {
                    ...
                    // Hide loading when the page starts to unload (form submission complete)
                    window.addEventListener('beforeunload', function() {
                        hideLoading();
                    }, { once: true });
                    
                    // Fallback: hide loading after 30 seconds in case something goes wrong
                    setTimeout(hideLoading, 30000);
                });
            });

            // Hide loading on any navigation or page load event
            ['load', 'pageshow', 'popstate', 'beforeunload'].forEach(event => {
                window.addEventListener(event, hideLoading);
            });
        });
    </script>
@endsection
