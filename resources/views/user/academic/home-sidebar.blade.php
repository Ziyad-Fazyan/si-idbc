...
<div class="sidebar-menu">
    ...
    @php
        $absensiActive = Str::startsWith(request()->path(), 'academic/absensi');
    @endphp
    ...
    <li class="list-none">
        ...
        <ul id="dropdown-absensi" class="dropdown-menu pl-12 mt-1 space-y-1 {{ $absensiActive ? '' : 'hidden' }}">
            <li
                class="{{ Route::is($prefix . 'absensi.dosen*') ? 'bg-green-50 text-green-700 font-semibold rounded-md' : '' }}">
            ...
            <li
                class="{{ Route::is($prefix . 'absensi.mahasiswa*') ? 'bg-green-50 text-green-700 font-semibold rounded-md' : '' }}">
                <a href="{{ route($prefix . 'absensi.mahasiswa.index') }}"
                    class="block py-2 pl-3 pr-4 text-sm rounded-md hover:bg-green-100 hover:text-green-700 transition-colors duration-200">
                    Absensi Mahasiswa
                </a>
            </li>
