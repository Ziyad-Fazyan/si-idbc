                    ...
                    <table class="min-w-full divide-y divide-[#E4E2DE]">
                        <thead class="bg-[#E4E2DE]">
                            <tr>
                                ...
                                <th scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-[#2E2E2E]">Nama Kelas</th>
                                <th scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-[#2E2E2E]">Nama Mata Kuliah</th>
                                <th scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-[#2E2E2E]">Dosen Pengajar</th>
                                <th scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-[#2E2E2E]">Metode Perkuliahan</th>
                                <th scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-[#2E2E2E]">Lokasi Perkuliahan</th>
                                <th scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-[#2E2E2E]">Tanggal Perkuliahan</th>
                                <th scope="col" class="px-3 py-3.5 text-center text-sm font-semibold text-[#2E2E2E]">Waktu Perkuliahan</th>
                        ...
                        <tbody class="divide-y divide-[#E4E2DE] bg-white">
                            @foreach ($jadkul as $key => $item)
                                <tr class="hover:bg-[#F3EFEA] transition-colors duration-200">
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-center text-[#2E2E2E]">{{ ++$key }}</td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-center text-[#2E2E2E]">{{ $item->kelas->code }}</td>
                                    <td class="px-3 py-4 text-sm text-center text-[#2E2E2E]">
                                        <div>{{ $item->matkul->name }}</div>
                                        <div class="text-[#3B3B3B] text-xs">{{ $item->pert_id . ' - ' . $item->bsks . ' SKS' }}</div>
                                    </td>
                                    <td class="px-3 py-4 text-sm text-center text-[#2E2E2E]">{{ $item->dosen->dsn_name }}</td>
                                    <td class="whitespace-nowrap px-3 py-4 text-sm text-center text-[#2E2E2E]">{{ $item->meth_id }}</td>
                                    <td class="px-3 py-4 text-sm text-center text-[#2E2E2E]">
                                        <div>{{ $item->ruang->gedung->name }}</div>
                                        <div class="text-[#3B3B3B] text-xs">{{ $item->ruang->name . ' - Lantai ' . $item->ruang->floor }}</div>
                                    </td>
                                    <td class="px-3 py-4 text-sm text-center text-[#2E2E2E]">
                                        <div>{{ $item->days_id }}</div>
                                        <div class="text-[#3B3B3B] text-xs">{{ \Carbon\Carbon::parse($item->date)->format('d M Y') }}</div>
                                    </td>
                                    <td class="px-3 py-4 text-sm text-center text-[#2E2E2E]">
                                        <div>{{ $item->start }}</div>
                ...
                <div class="md:hidden space-y-4">
                    @foreach ($jadkul as $key => $item)
                        <div class="bg-white border border-[#E4E2DE] rounded-lg shadow-sm overflow-hidden">
                            <div class="bg-[#0C6E71] px-4 py-2 text-white font-medium flex justify-between items-center">
                            ...
                            <div class="p-4 space-y-3 text-[#2E2E2E]">
                                <div>
                                    ...
                                    <div class="text-xs text-[#3B3B3B]">{{ $item->pert_id . ' - ' . $item->bsks . ' SKS' }}</div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div>{{ $item->ruang->gedung->name }}</div>
                                    <div class="text-xs text-[#3B3B3B]">{{ $item->ruang->name . ' - Lantai ' . $item->ruang->floor }}</div>
                                </div>
                                
                                <div>
                                    <div class="text-xs font-medium uppercase text-[#3B3B3B]">Tanggal Perkuliahan</div>
                                    <div>{{ $item->days_id }}</div>
                                    <div class="text-xs text-[#3B3B3B]">{{ \Carbon\Carbon::parse($item->date)->format('d M Y') }}</div>
                                </div>
