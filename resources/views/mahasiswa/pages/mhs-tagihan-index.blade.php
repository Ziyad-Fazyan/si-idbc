                            ...
                            <tr>
                                ...
                                <td class="d-flex justify-content-center align-items-center">
                                    <a href="{{ route('mahasiswa.home-tagihan-view', $item->code) }}"
                                        class="btn btn-outline-success"><i style="margin-right: 5px"
                                            class="fa-solid fa-money-bill-transfer"></i> Bayar Sekarang</a>
                ...
                <table class="table table-striped" id="table1">
                    ...
                    <tbody>
                        @foreach ($history as $key => $item)
                            <tr>
                                <td data-label="Number">{{ ++$key }}</td>
                                <td data-label="Kode Pembayaran"><span
                                        style="text-transform: uppercase">{{ $item->code }}</span></td>
                                <td data-label="Kode Tagihan"><span
                                        style="text-transform: uppercase">{{ $item->tagihan_code }}</span></td>
                                <td data-label="Nominal Bayar">Rp. {{ number_format($item->tagihan->price, 0, ',', '.') }}
                                </td>
                                <td data-label="Status">
                                    <a href="{{ route('mahasiswa.home-tagihan-invoice', $item->code) }}"
                                        class="btn btn-outline-success">{{ $item->stat === 1 ? 'PAID' : 'UN-PAID' }}</a>
                                </td>
                                {{-- <td class="d-flex justify-content-center align-items-center">
